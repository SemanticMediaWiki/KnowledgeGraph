...
KnowledgeGraphDialog = ( function () {
	let Config, CallbackActionProcess, CallbackOnSetup, CallbackInitializeResultsPanel;
	...
	MyDialog.prototype.initialize = function () {
		...
		function TabPanelOneLayout( name, config ) {
			...
			self.titleInputWidget.$input.on( 'input', function () {
				...
				const searchVal = val.replace( / /g, '_' );
				const ns = parseInt( self.namespaceDropdown.getValue() || 0, 10 );
				...
				new mw.Api().get( {
					...
					const menu = self.titleInputWidget.getMenu();
					menu.clearItems();
					menu.addItems( menuItems );
					menu.toggle( menuItems.length > 0 );
				} );
			} );
		}
		...
		const tabPanel1 = new TabPanelOneLayout( 'by-article' ),
	...
	MyDialog.prototype.initializeResultsPanel = function (
		...
		const panel = this.stackLayout.getItems()[ 1 ];
		this.stackLayout.setItem( panel );
	};

	// Specify processes to handle the actions.
	MyDialog.prototype.getActionProcess = function ( action ) {
		const ret = CallbackActionProcess(
			this, MyDialog.super.prototype.getActionProcess, action
		);
		if ( ret ) {
			return ret;
		}
	...
	function create(
		config, params, callbackActionProcess, callbackOnSetup, callbackInitializeResultsPanel
	) {
		Config = config;
